...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const BRIDGE_FIELDS = [
      ...
      { key: "trustedHosts", label: "受信代理 Host（逗号分隔）", hint: "例如 dsh.example.com。只有 Host 命中且带令牌的请求才会被桥接受", placeholder: "dsh.example.com" },
      ...
      return React.createElement(
        ...
        state.phase === "off"
          ? React.createElement(
              ...
              "当前为远程访问，且服务端未开启「受信代理远程桥」。请在这台运行 dsh 的机器上打开设置页（localhost），在「消息通道 → 受信代理远程桥」里开启并配置反代 + 令牌；或直接编辑 ~/.dsh/settings.yaml 的 im-gateway-* 段后重启。"
            )
          : null,
        state.phase === "ready"
          ? React.createElement(
