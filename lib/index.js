...
const NS_BRIDGE = settingsNamespace("im-gateway-bridge");
const SECTION_ORDER = 200;
// 受信代理远程桥：挂在本插件注册的 Web 路由上，只服务下面这些命名空间
// （不含 im-gateway-bridge 本身 —— 桥的开关只能在本机设置页改）。
const BRIDGE_PATH = "/chat-gateway-bridge";
const BRIDGE_SERVED = [NS_DINGTALK, NS_FEISHU, NS_WECOM, NS_QQ, NS_AGENT];
const BRIDGE_SECRET_FIELDS = {
  [NS_DINGTALK]: ["clientSecret"],
  [NS_FEISHU]: ["appSecret"],
  [NS_WECOM]: ["secret", "token", "encodingAesKey"],
  [NS_QQ]: ["accessToken"],
  [NS_AGENT]: [],
};
...
const QqConfig = z.object({
  ...
  accessToken: z.string().role("secret").default("").description("OneBot 访问令牌（留空则仅校验回环 Origin；建议填写，NapCat/LLOneBot 需配置相同 token）"),
});
...
// 受信代理远程桥：让远程浏览器也能读写本插件的设置。令牌只存服务端环境
// 变量，浏览器永远拿不到；桥只接受「回环来源 + Host 命中 + 令牌匹配」。
const BridgeConfig = z.object({
  enabled: z.boolean().default(false).description("启用受信代理远程桥"),
  trustedHosts: z.string().default("").description("受信代理的 Host，逗号分隔（如 dsh.example.com）"),
  tokenEnv: z.string().default("DSH_CHAT_GATEWAY_BRIDGE_TOKEN").description("存放共享令牌的环境变量名"),
});

// 行级 Config（cordis.patch.yml 的行不携带 config，仅作占位）
const Config = z.object({});

// Cordis 插件契约：name / Config / apply（见 dsh-TUI 插件指南）
const name = "dsh-chat-gateway";
...
function apply(ctx, config) {
  ...
  async function createAgent(convId) {
    ...
    const setup = async (agentCtx) => {
      ...
      // installModelSelection 动态导入 + 降级：较旧 DSH 版本的 dsh-agent 若
      // 不导出它，仍靠 agentOptions.provider/model 兜底（{{model}}/{{provider}}
      // 提示变量与 LLM 请求路由都读 agent.options，见 dsh-agent-loop）。
      try {
        const mod = await import("@deepseek-ai/dsh-agent");
        if (mod && typeof mod.installModelSelection === "function") {
          mod.installModelSelection(agentCtx, selection);
        }
      } catch (e) {
        warn("im-gateway: installModelSelection unavailable: " + (e?.message || String(e)));
      }
        ...
        if (req.method === "POST") {
          ...
          req.on("end", () => {
            try {
              ...
              const encrypt = m[1];
              if (!params.msg_signature || !params.timestamp || !params.nonce) { res.writeHead(400).end(); return; }
              if (wecomSignature(cfg.token, params.timestamp, params.nonce, encrypt) !== params.msg_signature) {
    ...
    function start(cfg, onMessage) {
      ...
      wss = new WebSocketServer({
        ...
        verifyClient: (info, cb) => {
          // 浏览器页面可跨域连 localhost WS（WS 无 CORS）——拒绝非回环 Origin，防恶意网页注入消息给 agent
          const origin = info.req.headers.origin;
          if (origin && !/^https?:\/\/(127\.0\.0\.1|localhost|\[::1\])(:\d+)?$/i.test(origin)) return cb(false);
          if (cfg.accessToken) {
            const url = new URL(info.req.url, "http://127.0.0.1");
            const tok = (info.req.headers.authorization || "").replace(/^Bearer\s+/i, "") || url.searchParams.get("access_token") || "";
            if (tok !== cfg.accessToken) return cb(false);
          }
          cb(true);
        },
      }, () => {
  ...
  // ================= 受信代理远程桥 =================
  const bridgeCfg = () => {
    const c = sources[NS_BRIDGE]?.() || {};
    return {
      enabled: c.enabled === true,
      trustedHosts: String(c.trustedHosts || "").split(",").map((s) => s.trim()).filter(Boolean),
      tokenEnv: c.tokenEnv || "DSH_CHAT_GATEWAY_BRIDGE_TOKEN",
    };
  };

  const isLoopback = (addr) => addr === "127.0.0.1" || addr === "::1" || addr === "::ffff:127.0.0.1";

  const redactBridge = (ns, value) => {
    const out = { ...(value || {}) };
    for (const key of BRIDGE_SECRET_FIELDS[ns] || []) delete out[key];
    return out;
  };
  ...
  const bridgeHandler = (req, res) => {
    const bc = bridgeCfg();
    if (!bc.enabled) { res.writeHead(404); res.end(); return; }
    if (!isLoopback(req.socket.remoteAddress)) { res.writeHead(403); res.end(); return; }
    const host = String(req.headers.host || "");
    if (bc.trustedHosts.length === 0 || !bc.trustedHosts.includes(host)) { res.writeHead(403); res.end(); return; }
    const expected = process.env[bc.tokenEnv];
    if (!expected || String(req.headers["x-dsh-chat-gateway-bridge-token"] || "") !== expected) { res.writeHead(403); res.end(); return; }

    let path;
    try { path = new URL(req.url || "/", "http://x").pathname; } catch (e) { path = "/"; }

    if (path === BRIDGE_PATH + "/describe" && req.method === "GET") {
      const settings = ctx.get("settings");
      writeJson(res, 200, {
        ok: true,
        writable: settings ? settings.writable !== false : false,
        namespaces: BRIDGE_SERVED.map((ns) => ({ ns, value: redactBridge(ns, sources[ns]?.() || {}) })),
      });
      return;
    }

    if (path === BRIDGE_PATH + "/mutate" && req.method === "POST") {
      // CSRF 防线：受信代理会为一切过路请求注入令牌，令牌无法区分用户意图。
      // 1) 强制 application/json —— 跨站表单/simple request 无法伪造，
      //    浏览器跨站发 JSON 必先预检，而本桥对 OPTIONS 一律 404;
      // 2) 拒绝明确的跨站 fetch 标记（现代浏览器均带 Sec-Fetch-Site）。
      const ct = String(req.headers["content-type"] || "");
      if (!ct.startsWith("application/json")) { writeJson(res, 415, { ok: false, error: "content-type must be application/json" }); return; }
      const sfs = String(req.headers["sec-fetch-site"] || "");
      if (sfs && sfs !== "same-origin" && sfs !== "none") { writeJson(res, 403, { ok: false, error: "cross-site request rejected" }); return; }
      let body = "";
      req.on("data", (chunk) => {
        body += chunk;
      ...
      req.on("end", async () => {
        try {
          ...
          if (!BRIDGE_SERVED.includes(ns) || field === "") { writeJson(res, 400, { ok: false, error: "bad request" }); return; }
          const settings = ctx.get("settings");
          if (!settings) { writeJson(res, 503, { ok: false, error: "settings service unavailable" }); return; }
          await settings.mutate(ns, [{ op: "set", path: [field], value: payload.value }]);
          writeJson(res, 200, { ok: true, value: redactBridge(ns, sources[ns]?.() || {}) });
        } catch (e) {
  ...
  // 桥路由注册必须等待 webServer 就绪：插件自身的 apply 可能在 webServer
  // 挂载前运行，用 ctx.get 会静默拿到 undefined。ctx.inject 会在服务出现后
  // 回调，且子上下文随本插件卸载自动清理路由。
  ctx.inject(["webServer"], (childCtx) => {
    const unregister = childCtx.webServer.register({
      kind: "prefix",
      path: BRIDGE_PATH,
      handler: bridgeHandler,
    });
    log("im-gateway: remote bridge route registered at " + BRIDGE_PATH);
    childCtx.effect(() => unregister, "chat-gateway: bridge route");
  });
...
export { name, Config, apply, inject };
